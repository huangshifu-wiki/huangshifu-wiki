...
- 为复杂逻辑添加适量简洁注释，说明代码做什么；无特殊情况不必说明为什么
...
- ROI 优先：测试投在出错代价高、影响大的地方——权限、数据安全、核心流程。
- 只测行为，不测实现。
- 分层覆盖：
  - 单元：纯逻辑、算法、复杂 Hook/Reducer。不测胶水代码、配置。
  - 集成/API：每端点正常响应 + 典型错误码(4xx/5xx)、权限边界。不穷举参数组合。
  - E2E：仅核心黄金流程 + 关键失败路径。不测 UI 细节和页面元素存在性。
- 类型即护栏：TypeScript 与 API 契约已拦截大量低级错误，不必重复验证。
- 覆盖率是雷达：未覆盖代码先判重要性，不硬凑测试。现有门槛是底线，不是撒网的理由。
- 三问决策：错了会痛？坏了难察觉？→ 测；测试脆弱易碎且无真正 bug（改点实现就挂，但无真正bug）？→ 不测。
